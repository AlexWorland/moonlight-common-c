...
int performRtspHandshake(PSERVER_INFORMATION serverInfo) {
    ...
    if (AppVersionQuad[0] >= 5) {
        ...
        // Parse Apollo feature flags if present
        char* apolloFeatures = getOptionContent(response.options, "x-apollo-features");
        if (apolloFeatures != NULL && ListenerCallbacks.rtspFeatures != NULL) {
            ListenerCallbacks.rtspFeatures(apolloFeatures);
        }
